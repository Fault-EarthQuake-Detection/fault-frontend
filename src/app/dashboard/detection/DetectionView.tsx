  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="space-y-6 animate-fade-in pb-10">
        <div className="text-center space-y-2">
          <h2 className="text-xl font-bold text-gray-800 flex items-center justify-center gap-2">Hasil Analisis Terpadu</h2>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-90 leading-relaxed">
                {isDanger ? "Indikasi kuat sesar aktif dari visual batuan DAN lokasi geografis." : isWarning ? "Terdapat potensi bahaya dari salah satu indikator (Visual atau Lokasi). Harap waspada." : "Tidak terdeteksi ancaman sesar signifikan pada visual maupun lokasi."}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl border shadow-sm overflow-hidden h-fit">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="border-t pt-3 space-y-3 text-sm">
                <p className="italic text-gray-600 bg-gray-50 p-2 rounded">&quot;{locationData?.status}&quot;</p>
                <div className="flex justify-between"><span className="text-gray-600">Sesar Terdekat:</span><span className="font-medium text-right">{locationData?.nama_patahan || "-"}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Jarak:</span><span className="font-medium">{locationData?.jarak_km ? `${locationData.jarak_km} km` : "-"}</span></div>
              </div>
            </div>
          </div>
        </div>
            ...
            {impactList.length > 0 ? (
                <div className="space-y-3">
                    <p className="text-sm text-gray-500 mb-2">
                        Ditemukan <strong>{impactList.length} infrastruktur penting</strong> di sekitar titik deteksi ini:
                    </p>
                    <div className="grid gap-3 sm:grid-cols-2">
                        {impactList.map((item) => (
                            <div key={item.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border hover:bg-red-50 transition border-l-4 border-l-gray-300 hover:border-l-red-500">
                                <div className="h-10 w-10 rounded-full bg-white flex items-center justify-center shadow-sm shrink-0">
                                    {item.type === 'school' && <GraduationCap className="h-5 w-5 text-orange-600" />}
                                    {item.type === 'hospital' && <Stethoscope className="h-5 w-5 text-red-600" />}
                                    {item.type === 'government' && <Landmark className="h-5 w-5 text-blue-600" />}
                                </div>
                                <div className="overflow-hidden">
                                    <p className="text-sm font-bold text-gray-800 truncate" title={item.name}>{item.name}</p>
                                    <p className="text-xs text-gray-500 flex items-center gap-1">
                                        Jarak: <span className="font-mono font-bold text-red-600">{item.distance}m</span>
                                    </p>
                                </div>
                            </div>
                        ))}
                ...
                <div className="text-center py-8 bg-green-50 rounded-xl border border-green-100">
                    ...
                    <p className="text-sm text-green-600">Tidak ditemukan sekolah, rumah sakit, atau kantor pemerintah dalam radius 1 KM.</p>
                </div>
            )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 pt-6">
          ...
          <button onClick={handleSave} disabled={saving || isSaved} className={`flex items-center justify-center gap-2 px-8 py-3 rounded-xl font-bold text-white shadow-lg transition w-full sm:w-auto ${isSaved ? "bg-green-600 hover:bg-green-700 cursor-default" : "bg-orange-700 hover:bg-orange-800"} disabled:opacity-70 disabled:cursor-not-allowed`}>
            {saving ? (<span className="flex items-center gap-2"><Loader2 className="animate-spin h-4 w-4" /> Menyimpan...</span>) : isSaved ? (<span className="flex items-center gap-2"><CheckCircle className="h-4 w-4" /> Tersimpan</span>) : (<span className="flex items-center gap-2"><Save className="h-4 w-4" /> Simpan Laporan & Dampak</span>)}
          </button>
        </div>
      </div>
    );
